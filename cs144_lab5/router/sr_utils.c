    ...
    if (len >= cumulative_sz) {
      ...
      if (ip_cksum(ipHdr, sizeof(sr_ip_hdr_t)) == ipHdr->ip_sum) {
        ...
        if (ipHdr->ip_p == ip_protocol_icmp) {
          ...
          sr_tcp_hdr_t *tcpHdr = (sr_tcp_hdr_t *) (packet + cumulative_sz);
          cumulative_sz += sizeof(sr_tcp_hdr_t);

          if (len >= cumulative_sz) {
            if (tcp_cksum(ipHdr, tcpHdr, len) == tcpHdr->sum) {
              return 1;
            }
          }
        } else {
...
void print_hdr_icmp(uint8_t *buf) {
  ...
  fprintf(stderr, "\theader extra1: %d\n", icmp_hdr->icmp_etc_hdr_1);
  fprintf(stderr, "\theader extra2: %d\n", icmp_hdr->icmp_etc_hdr_2);
}
...
/* Prints out the string representation of the tcp state enum. */
void print_tcp_state(sr_tcp_state tcp_state) {
  switch(tcp_state) {
    case CLOSED:
      printf("CLOSED\n");
      break;
