 ...
 *---------------------------------------------------------------------*/
 ...
 void sr_handlepacket(struct sr_instance* sr,
  ...
  if (is_packet_valid(packet, len)) {
    ...
    printf("*** -> WARNING! Failed packet validation function. Packet was discarded.\n");
  }
...
void sr_handle_ip_packet(struct sr_instance *sr,
  ...
  if (myInterface == NULL && lpmEntry == NULL) {
    ...
    if (sr->nat_mode) {
      ...
      printf("## -> NAT MODE\n");
      if (sr_nat_is_iface_internal(interface)) {
          ...
          if (ipProtocol == ip_protocol_icmp) {
            ...
            if (nat_lookup != NULL) {
              if (is_icmp_echo_reply(icmp_hdr)) {
                ...
                switch_route(sr, packet, len, srcAddr, destAddr, interface, eHdr, ipHdr, sr_get_lpm_entry(sr->routing_table, ipHdr->ip_dst));
              }
            }
          } else if (ipProtocol == ip_protocol_tcp) {
            printf("## -> (EX-IN) TCP\n");
